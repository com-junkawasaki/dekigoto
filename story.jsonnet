{
  ...
  examples: {
    ...
    nextjs_libsql: {
      id: 'nextjs_libsql_example_v2',
      description: 'A Next.js & LibSQL application showcasing a highly abstracted, schema-driven ActorDB client.',
      dependencies: ['secure_client_framework', 'domain_schema'],
      outputs: ['type_safe_api_routes', 'secure_queries', 'declarative_domain_logic'],
      merkle_hash: 'sha256:nextjs_libsql_example_v2',
      architecture: {
        pattern: 'Schema-driven CQRS with Capability-Based Security',
        layers: {
          client_framework: {
            id: 'secure_client_framework',
            description: 'Provides a Supabase/Drizzle-like fluent API, dynamically generated from the domain schema and user capabilities.',
            components: [
              'SecureClientFactory',
              'ApplicationBuilder',
              'ReducerAggregate',
              'TypedActorManager (State-Session-ORM)',
              'CommandBus (Mediator)',
            ],
          },
          application_logic: {
            id: 'domain_schema',
            description: 'The single source of truth for the application domain, defined declaratively.',
            components: [
              'DomainSchema (actors, commands, queries, capabilities)',
              'UseCases (Command/Query Handlers)',
              'Aggregates (Declarative Reducers)',
              'DomainEvents',
            ],
          },
          presentation_layer: {
            id: 'nextjs_api_routes',
            description: 'Thin API routes that create a per-request secure client and dispatch commands/queries.',
          },
        },
      },
    },
  ...
  version: '1.1.0',
  merkle_root: 'sha256:actordb_dag_v1.1_with_v2_example',
